...
def uniform(n_samples, bounds, method='sobol', normalize=False, scramble=True, seed=None):
    '''
    Objective:
        - Generates a uniformly distributed sequence using the specified QMC method.
    Inputs:
        - n_samples: Number of samples to generate.
        - bounds: Range to sample over.
        - sequence: String specifying the sampling method ('sobol', 'random', 'lhs', 'halton').
        - normalize: Boolean, if True keeps samples normalized to [0,1].
        - scramble: Boolean, to apply random noise (scrambling) to the sequence.
        - seed: Random seed.
    Outputs:
    ...
    if method == 'random':
        ...
        samples_unit = rng.random((n_samples, n_dimensions))
        
    # QMC generation
    else:
        if method == 'sobol':
            sampler = stats.qmc.Sobol(d=n_dimensions, scramble=scramble, seed=seed)
        elif method in ['lhs', 'latin_hypercube']:
            sampler = stats.qmc.LatinHypercube(d=n_dimensions, scramble=scramble, seed=seed)
        elif method == 'halton':
            sampler = stats.qmc.Halton(d=n_dimensions, scramble=scramble, seed=seed)
        else:
            raise ValueError(f"Sampling method '{method}' is not supported. Choose 'sobol', 'random', 'lhs', 'halton'.")
            
        samples_unit = sampler.random(n=n_samples)
    
    # scale bounds if provided
    if not normalize:
        sequence = stats.qmc.scale(samples_unit, bounds[:, 0], bounds[:, 1])
    else:
        sequence = samples_unit
...
def fit_pca_for_cluster(cluster_samples, current_origin, 
    ...
    '''Diagonal (variance) only; no rotation (orthogonal through each cluster).
    Covariance = sampling noise.'''
    n_cluster_samples = len(cluster_samples)
    
    # extreme fallback for tiny clusters (impossible to get variance)
    if n_cluster_samples < 2:
        return {
            'origin': current_origin,
            'components': np.eye(n_dimensions),
            'variances': np.ones(n_dimensions) * initial_samples_std
        }

    # center samples
    centered = cluster_samples - np.mean(cluster_samples, axis=0)
    
    # calculate variance of each dimension (no correlation in initial QMC sequence); ensures orthogonal hyperellipsoids
    variances = np.var(cluster_samples, axis=0, ddof=1)
    
    return {
        'origin': current_origin,
        'components': np.eye(n_dimensions), # no rotation
        'variances': np.maximum(variances, 0.0) # keep the hyperellipsoid shape
    }

def sample_hyperellipsoid(n_dimensions, n_samples_in_ellipsoid, 
                               origin, variances, 
                               scaling_factor, radius_qmc_sequence, seed=None):
    '''
    Objective:
    ...
    # calculate axis lengths from variance, scaled by scaling factor
    axis_lengths = np.sqrt(variances + epsilon) * scaling_factor
    
    # scale the hypersphere samples by the radial scaling, axis lengths, and add centroid
    ellipsoid_samples = (unit_sphere_samples * (random_radii[:, np.newaxis] * axis_lengths)) + origin
...
def sample(n_samples, bounds,
    ...
    # n_initial_clusters scaling
    if n_initial_clusters is None:
        n_initial_clusters = 8 # reduced from 100 to increase resolution within each cluster
    n_initial_clusters = int(n_initial_clusters)
    
    # number of qmc samples scaling
    n_initial_qmc_max = 2**15
    if n_initial_qmc is None:
        min_qmc_dimensions = int(2**np.ceil(np.log2(n_dimensions*100)))
    ...
    if weights:
        ...
        for dim, info in weights.items():
            ...
            std = info['std']
            if not std > 0:
                raise ValueError(f'Gaussian weight stdevs must be > 0.')
                return None
    ...
    kmeans_initial = MiniBatchKMeans(n_clusters=n_initial_clusters, random_state=seed, n_init=4)
    ...
    radius_qmc_sequence_base = radius_qmc_sampler.random(n=int(n_samples * 2)) # generate extra samples
    radius_start_idx = 0
